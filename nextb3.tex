...
It follows that the background rejection of a future ton-scale HPXe using a magnetic field could be up to a factor of 10 better than the current rejection factor expected by the NEXT-100 experiment while keeping an efficiency in the range of 20-25\% (NEXT-100 is expected to operate with a signal efficiency of approximately 30\% \cite{NEXT_2013} without considering additional rejection provided by the present analysis). The resulting background rate would be around $5 \cdot 10^{-5}$ \ckky\ or about a half count per ton and year in the ROI, assuming a resolution of 0.5\% FWHM at \Qbb. This is enough, for most nuclear matrix element sets, to fully cover the inverse hierarchy  in a few years. 
